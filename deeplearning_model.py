...
def deeplearning_model():
    ...
    def prepare_dl_data(df, target_col="y", test_ratio=0.2, seq_length=20):
        ...
        feature_cols = [
            col for col in df.columns if col not in ["ds", "unique_id", target_col]
        ]
        ...
        if train_size < seq_length + 10:
            raise ValueError(
                f"Not enough training data! Total samples: {total_samples}, need at least {seq_length + test_size + 10}"
            )
    ...
    model_crypto = LSTMPredictor(
        input_size=input_size_crypto, hidden_size=64, num_layers=2
    )
    ...
    metrics_crypto = calculate_all_metrics(
        actual_crypto, pred_crypto, "LSTM_Time+Crypto"
    )
    ...
    improvement_r2 = (
        (metrics_crypto["R²"] - metrics_time["R²"]) / abs(metrics_time["R²"] + 0.0001)
    ) * 100
    improvement_mae = (
        (metrics_time["MAE"] - metrics_crypto["MAE"]) / metrics_time["MAE"]
    ) * 100
    ...
    if (
        metrics_crypto["R²"] > metrics_time["R²"]
        and metrics_crypto["Correlation"] > metrics_time["Correlation"]
    ):
        print("\n✅ CONCLUSION: Crypto features IMPROVE deep learning predictions!")
        print(f"   R² improved by {improvement_r2:.1f}%")
        print(
            f"   Model with crypto captures {metrics_crypto['R²'] * 100:.1f}% of variance"
        )
    else:
    ...
    ax.plot(
        test_dates,
        pred_time,
        label=f"LSTM Time-Only (R²={metrics_time['R²']:.3f})",
        color="blue",
        linewidth=2,
        linestyle="--",
        marker="s",
    ...
    ax.plot(
        test_dates,
        pred_crypto,
        label=f"LSTM Time+Crypto (R²={metrics_crypto['R²']:.3f})",
        color="red",
        linewidth=2,
        linestyle="-",
        marker="x",
