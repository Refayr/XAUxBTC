...
def naive_model():
    print("\n" + "=" * 70)
    print("Load dataset for forecasting")
    print("=" * 70)
    ...
    # Plot training data
    plt.plot(
        train["ds"], train["y"], label="Train", color="blue", linewidth=2, alpha=0.6
    )
    ...
    split_summary = (
        split_evaluation.drop(["unique_id"], axis=1)
        .groupby("metric")
        .mean()
        .reset_index()
    )
    ...
    ax.set_title(
        "Simple Train/Test Split - Last 30 Days Prediction",
        fontsize=14,
        fontweight="bold",
    )
    ...
    # Plot actual values
    ax.plot(
        df_au["ds"], df_au["y"], label="Actual", color="black", linewidth=2, alpha=0.7
    )
    ...
    ax1.set_title(
        "MAE: Simple Split vs Cross-Validation", fontsize=13, fontweight="bold"
    )
    ax1.set_xticks(x)
    ...
    ax2.set_title(
        "RMSE: Simple Split vs Cross-Validation", fontsize=13, fontweight="bold"
    )
    ax2.set_xticks(x)
    ...
    ax.set_title(
        "Top Performing Models - MAE Comparison", fontsize=14, fontweight="bold"
    )
    ax.set_xticks(x_pos)
    ...
    print(
        f"Cross-Validation: {best_cv} (MAE: {cv_model_pivot.loc[best_cv, 'mae']:.4f})"
    )
