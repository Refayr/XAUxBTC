...
exog_df['months_since_start'] = (
    (exog_df['ds'].dt.year - exog_df['ds'].min().year) * 12 + 
    (exog_df['ds'].dt.month - exog_df['ds'].min().month)
)
...
exog_df['days_to_holiday'] = exog_df['ds'].apply(
    lambda x: (holidays[holidays > x].min() - x).days if len(holidays[holidays > x]) > 0 else 365
)
...
# Plot training data
ax1.plot(train_lr['ds'], train_lr['y'], 
        color='blue', linewidth=2, alpha=0.6, label='Train')

# Plot test actual
ax1.plot(test_lr['ds'], test_lr['y'], 
        color='black', linewidth=3, marker='o', markersize=7,
        label='Actual (Test)', zorder=5)

# Plot Linear Regression predictions
ax1.plot(test_lr_eval['ds'], test_lr_eval['LinearRegression'], 
        color='red', linewidth=2.5, linestyle='--', marker='x', markersize=6,
        label='Linear Regression', alpha=0.8, zorder=4)

# Plot Ridge predictions
ax1.plot(test_lr_eval['ds'], test_lr_eval['Ridge'], 
        color='orange', linewidth=2.5, linestyle='--', marker='^', markersize=5,
        label='Ridge Regression', alpha=0.8, zorder=4)

# Add vertical line at train/test split
if len(train_lr) > 0:
    ax1.axvline(x=train_lr['ds'].max(), color='red', linestyle=':', 
               linewidth=2, alpha=0.6, label='Train/Test Split')

ax1.set_xlabel('Date', fontsize=12, fontweight='bold')
ax1.set_ylabel('Gold Price', fontsize=12, fontweight='bold')
ax1.set_title('Linear Regression Models - Full Timeline', fontsize=14, fontweight='bold')
...
ax2.plot(test_lr_eval['ds'], test_lr_eval['y'], 
        color='black', linewidth=3.5, marker='o', markersize=9,
        label='Actual', zorder=5)

ax2.plot(test_lr_eval['ds'], test_lr_eval['LinearRegression'], 
        color='red', linewidth=3, linestyle='--', marker='x', markersize=8,
        label='Linear Regression', alpha=0.8, zorder=4)

ax2.plot(test_lr_eval['ds'], test_lr_eval['Ridge'], 
        color='orange', linewidth=3, linestyle='--', marker='^', markersize=7,
        label='Ridge', alpha=0.8, zorder=4)
...
ax3.plot(test_lr_eval['ds'], lr_errors, 
        color='red', linewidth=2.5, marker='o', markersize=5,
        label='Linear Reg Error', alpha=0.7)

ax3.plot(test_lr_eval['ds'], ridge_errors, 
        color='orange', linewidth=2.5, marker='^', markersize=5,
        label='Ridge Error', alpha=0.7)
...
# Overall title
fig.suptitle('Linear Regression Forecasting Results', 
...
plt.title('Top 15 Feature Importance (Linear Regression)', fontsize=14, fontweight='bold')
...
if 'split_evaluation' in locals() or 'split_evaluation' in globals():
    try:
        ...
        sarima_metrics = split_evaluation[split_evaluation['metric'] == 'mae']['SARIMA'].values[0]
        ...
        arima_rmse = split_evaluation[split_evaluation['metric'] == 'rmse']['ARIMA'].values[0]
        sarima_rmse = split_evaluation[split_evaluation['metric'] == 'rmse']['SARIMA'].values[0]
        comparison_data['RMSE'].extend([arima_rmse, sarima_rmse])
        
        # Get MAPE
        arima_mape = split_evaluation[split_evaluation['metric'] == 'mape']['ARIMA'].values[0]
        sarima_mape = split_evaluation[split_evaluation['metric'] == 'mape']['SARIMA'].values[0]
