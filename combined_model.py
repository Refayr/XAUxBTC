...
arima_pred_time = train_arima(
    data_time['arima_train'], 
    data_time['arima_test'],
    X_test=data_time['arima_test']
)
...
with torch.no_grad():
    ...
    lstm_pred_time = data_time['target_scaler'].inverse_transform(lstm_pred_time_scaled).flatten()
...
with torch.no_grad():
    ...
    lstm_pred_crypto = data_crypto['target_scaler'].inverse_transform(lstm_pred_crypto_scaled).flatten()
...
w_arima_time, w_lstm_time = calculate_weights(arima_pred_time_aligned, lstm_pred_time, actual_time)
w_arima_crypto, w_lstm_crypto = calculate_weights(arima_pred_crypto_aligned, lstm_pred_crypto, actual_crypto)

ensemble_weighted_time = (w_arima_time * arima_pred_time_aligned + 
                          w_lstm_time * lstm_pred_time)
ensemble_weighted_crypto = (w_arima_crypto * arima_pred_crypto_aligned + 
                            w_lstm_crypto * lstm_pred_crypto)
...
# Time-Only Dataset
results_time = []
results_time.append(calculate_metrics(actual_time, arima_pred_time_aligned, 'ARIMA_TimeOnly'))
results_time.append(calculate_metrics(actual_time, lstm_pred_time, 'LSTM_TimeOnly'))
results_time.append(calculate_metrics(actual_time, ensemble_avg_time, 'Ensemble_Avg_TimeOnly'))
results_time.append(calculate_metrics(actual_time, ensemble_weighted_time, 'Ensemble_Weighted_TimeOnly'))

# Time+Crypto Dataset
results_crypto = []
results_crypto.append(calculate_metrics(actual_crypto, arima_pred_crypto_aligned, 'ARIMA_Time+Crypto'))
results_crypto.append(calculate_metrics(actual_crypto, lstm_pred_crypto, 'LSTM_Time+Crypto'))
results_crypto.append(calculate_metrics(actual_crypto, ensemble_avg_crypto, 'Ensemble_Avg_Time+Crypto'))
results_crypto.append(calculate_metrics(actual_crypto, ensemble_weighted_crypto, 'Ensemble_Weighted_Time+Crypto'))
...
ax.plot(dates_time, actual_time, label='Actual', 
        color='black', linewidth=3, marker='o', markersize=6, zorder=10)
ax.plot(dates_time, arima_pred_time_aligned, label='ARIMA', 
        color='blue', linewidth=2, linestyle='--', alpha=0.7)
ax.plot(dates_time, lstm_pred_time, label='LSTM', 
        color='green', linewidth=2, linestyle='--', alpha=0.7)
ax.plot(dates_time, ensemble_avg_time, label='Ensemble (Avg)', 
        color='red', linewidth=2, linestyle='-', marker='x', markersize=5, alpha=0.8)
ax.plot(dates_time, ensemble_weighted_time, label='Ensemble (Weighted)', 
        color='purple', linewidth=2, linestyle='-', marker='+', markersize=6, alpha=0.8)

ax.set_xlabel('Date', fontsize=13, fontweight='bold')
ax.set_ylabel('Gold Price (xAU)', fontsize=13, fontweight='bold')
ax.set_title(f'Time-Only Models: ARIMA + LSTM Ensemble\nBest: {best_time["Model"]} (R²={best_time["R²"]:.3f})', 
...
ax.plot(dates_crypto, actual_crypto, label='Actual', 
        color='black', linewidth=3, marker='o', markersize=6, zorder=10)
ax.plot(dates_crypto, arima_pred_crypto_aligned, label='ARIMA', 
        color='blue', linewidth=2, linestyle='--', alpha=0.7)
ax.plot(dates_crypto, lstm_pred_crypto, label='LSTM', 
        color='green', linewidth=2, linestyle='--', alpha=0.7)
ax.plot(dates_crypto, ensemble_avg_crypto, label='Ensemble (Avg)', 
        color='red', linewidth=2, linestyle='-', marker='x', markersize=5, alpha=0.8)
ax.plot(dates_crypto, ensemble_weighted_crypto, label='Ensemble (Weighted)', 
        color='purple', linewidth=2, linestyle='-', marker='+', markersize=6, alpha=0.8)

ax.set_xlabel('Date', fontsize=13, fontweight='bold')
ax.set_ylabel('Gold Price (xAU)', fontsize=13, fontweight='bold')
ax.set_title(f'Time+Crypto Models: ARIMA + LSTM Ensemble\nBest: {best_crypto["Model"]} (R²={best_crypto["R²"]:.3f})', 
             fontsize=15, fontweight='bold')
ax.legend(fontsize=10, loc='best', framealpha=0.9)
ax.grid(True, alpha=0.3, linestyle=':')
plt.xticks(rotation=45)
...
axes[0].bar(x - width/2, r2_time, width, label='Time-Only', color='steelblue', alpha=0.8)
axes[0].bar(x + width/2, r2_crypto, width, label='Time+Crypto', color='coral', alpha=0.8)
...
axes[1].bar(x - width/2, mae_time, width, label='Time-Only', color='steelblue', alpha=0.8)
axes[1].bar(x + width/2, mae_crypto, width, label='Time+Crypto', color='coral', alpha=0.8)
