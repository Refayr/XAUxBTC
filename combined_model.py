...
def combined_model():
    def seed_everything(seed: int = 42):
        ...
        os.environ["PYTHONHASHSEED"] = str(
            seed
        )  # helps make hashing-based ops reproducible
    ...
    def prepare_data_for_ensemble(df, test_size=60, seq_length=20):
        ...
        feature_cols = [
            col for col in df.columns if col not in ["ds", "unique_id", "y"]
        ]
    ...
    with torch.no_grad():
        ...
        lstm_pred_time = (
            data_time["target_scaler"]
            .inverse_transform(lstm_pred_time_scaled)
            .flatten()
        )
    ...
    print(
        f"  Time+Crypto weights: ARIMA={w_arima_crypto:.3f}, LSTM={w_lstm_crypto:.3f}"
    )
    ...
    results_time.append(
        calculate_metrics(
            actual_time, ensemble_weighted_time, "Ensemble_Weighted_TimeOnly"
        )
    )
    ...
    results_crypto.append(
        calculate_metrics(
            actual_crypto, ensemble_avg_crypto, "Ensemble_Avg_Time+Crypto"
        )
    )
    results_crypto.append(
        calculate_metrics(
    ...
    if best_crypto["R²"] > best_time["R²"]:
        improvement = (
            (best_crypto["R²"] - best_time["R²"]) / abs(best_time["R²"])
        ) * 100
    ...
    axes[0].set_title(
        "R² Comparison (Higher is Better)", fontsize=13, fontweight="bold"
    )
    axes[0].set_xticks(x)
    ...
    axes[1].set_title(
        "MAE Comparison (Lower is Better)", fontsize=13, fontweight="bold"
    )
    axes[1].set_xticks(x)
    ...
    # Format nicely
    for dataset, df in [
        ("Time-Only", results_time_df),
        ("Time+Crypto", results_crypto_df),
    ]:
        print(f"\n{dataset}:")
